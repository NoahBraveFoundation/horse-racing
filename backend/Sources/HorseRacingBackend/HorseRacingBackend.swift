  ...
  static func main() async throws {
    ...
    do {
      ...
      let corsConfiguration = CORSMiddleware.Configuration(
        allowedOrigin: .any([
          "https://horses.noahbrave.org", "http://localhost:5173",
      ...
      self.storage[PassBuilderServiceKey.self]
        ?? {
          guard
            ...
            fatalError(
              "Missing Wallet env vars (PASS_*). Configure certificates to enable pass generation.")
          }
          guard
...
struct UserSessionLoaderMiddleware: AsyncMiddleware {
  func respond(to request: Request, chainingTo next: AsyncResponder) async throws -> Response {
    if let userIDString = request.session.data["userID"],
...
func configurePassesService(_ app: Application) throws {
  ...
  guard
    ...
    app.logger.warning(
      "Required: PASS_CERT_PEM, PASS_WWDR_PEM, PASS_PRIVATE_KEY_PEM, PASS_CERT_PASSWORD")
...
func seedSampleData(_ app: Application) async throws {
  ...
  if try await User.query(on: app.db).filter(\.$email == "austinjevans@me.com").first() == nil {
    let admin = User(
      email: "austinjevans@me.com", firstName: "Austin", lastName: "Evans", isAdmin: true)
